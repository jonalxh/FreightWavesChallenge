<template>
  <q-page class="q-pa-md">
    ...
    <table-container
      ...
      @row-edit="showEditDialog($event, false)"
      ...
      <q-card-section v-if="selectedUser">
        <q-form>
          <div class="row q-col-gutter-md">
            ...
            <template
              v-for="(fieldValue, fieldName, fieldIndex) in selectedUser"
              :key="fieldIndex"
            >
              <q-input
                class="col-12"
                v-model="selectedUser[fieldName]"
                v-if="
                  fieldName !== 'id' &&
                  ['number', 'string'].includes(typeof fieldValue)
                "
                :readonly="readonly"
                label-slot
                dense
              ...
              <template v-else-if="typeof fieldValue === 'object'">
                ...
                <div class="col-12 text-bold text-capitalize">
                  {{ fieldName }}
                </div>

                <template
                  v-for="(objValue, objName, objIndex) in fieldValue"
                  :key="objIndex"
                >
                  <q-input
                    class="col-12"
                    v-model="selectedUser[fieldName][objName]"
                    v-if="objName !== 'geo'"
                    :readonly="readonly"
                    label-slot
                    dense
...
export default defineComponent({
  ...
  setup() {
    ...
    const submitUserEdition = () => {
      ...
      api
        .patch(`users/${selectedUser.value.id}`, selectedUser.value)
        .then((res) => {
